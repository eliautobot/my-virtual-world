...
assert.equal(serverPingPongLeft.target?.pingPongSide, 'left', 'left player target must preserve its side throughout approach');
assert.equal(serverPingPongLeft.target?.paddleColor, 0xf44336, 'left player target must preserve the red paddle throughout approach');
assert.equal(serverPingPongRight.target?.pingPongSide, 'right', 'right player target must preserve its side throughout approach');
assert.equal(serverPingPongRight.target?.paddleColor, 0x2196f3, 'right player target must preserve the blue paddle throughout approach');
const serverPingPongLeftVisual = JSON.parse(pingPongRoom.state.agents.get('left-player').visualStateJson || '{}');
const serverPingPongRightVisual = JSON.parse(pingPongRoom.state.agents.get('right-player').visualStateJson || '{}');
assert.equal(serverPingPongLeftVisual.activity?.source, 'server-pingpong-runtime', 'left visual state should identify server ping-pong ownership');
assert.equal(serverPingPongRightVisual.activity?.source, 'server-pingpong-runtime', 'right visual state should identify server ping-pong ownership');
assert.equal(serverPingPongLeftVisual.activity?.paddleColor, 0xf44336, 'left approach visual must remain red before play begins');
assert.equal(serverPingPongRightVisual.activity?.paddleColor, 0x2196f3, 'right approach visual must remain blue before play begins');
assert.equal(serverPingPongRightVisual.pingPongSide, 'right', 'right approach visual must expose stable blue-side metadata');
