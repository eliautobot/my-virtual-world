...
type ClaimedWorldBinding = {
  baseUrl: string;
  bridgeSecret: string;
};

function stableWorldToken(worldId: unknown): string {
...
function worldRegistryPath(): string {
...
function claimedWorldBinding(context: BoundContext): ClaimedWorldBinding {
  const agentId = String(context.agentId || "").trim();
  const sessionKey = String(context.sessionKey || "").trim();
  if (!agentId || !sessionKey) return { baseUrl: "", bridgeSecret: "" };
  try {
    ...
    if (!worldToken || !/^\d{2,5}$/.test(port)) return { baseUrl: "", bridgeSecret: "" };
    if (!sessionKey.endsWith(`:vw-live-world-${worldToken}`)) return { baseUrl: "", bridgeSecret: "" };
    const numericPort = Number(port);
    if (!Number.isInteger(numericPort) || numericPort < 1 || numericPort > 65535) return { baseUrl: "", bridgeSecret: "" };
...
function normalizedBaseUrl(config: PluginConfig, binding: ClaimedWorldBinding): string {
  return String(
    config.baseUrl
...
function boundTool(
  ...
  return tool({
    ...
    factory({ config, toolContext }: { config: PluginConfig; toolContext: BoundContext }) {
      return {
        name: definition.name,
        label: definition.label,
        description: definition.description,
        parameters: definition.parameters,
        async execute(_toolCallId: string, params: Record<string, unknown>, _onUpdate?: unknown, runContext?: { signal?: AbortSignal }) {
