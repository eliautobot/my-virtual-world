...
LIVE_AGENT_PIANO_RUNTIME_TRACE_VERSION = "agent-live-mode-piano-runtime-trace/v1"
...
LIVE_AGENT_LOOP_DEFAULTS = {
    ...
    "planMaxRetries": 2,
    "turnRetention": 80,
    "pianoRuntimeTraceRetention": 240,
    "turnTimeoutSec": 240,
...
def _live_agent_piano_contract_readiness(memory_counts):
    return {
        "perception": {
...
def _live_agent_piano_architecture_metrics(loop_state, completed_backend_actions, memory_counts, communication_events, relationships, animation_event_names):
    runtime = _normalize_live_agent_piano_runtime(loop_state.get("pianoRuntime") if isinstance(loop_state, dict) else None)
    traces = [trace for trace in (runtime.get("traces") or []) if isinstance(trace, dict)]
    contract_readiness = _live_agent_piano_contract_readiness(memory_counts)
    modules = {}
    for module_name in LIVE_AGENT_PIANO_MODULES:
...
def default_live_agent_loop_state():
    ...
    return {
        ...
        "pianoRuntime": {
            "schemaVersion": LIVE_AGENT_PIANO_RUNTIME_TRACE_VERSION,
            "moduleOrder": list(LIVE_AGENT_PIANO_MODULES),
            "traceRetention": LIVE_AGENT_LOOP_DEFAULTS["pianoRuntimeTraceRetention"],
            "nextSequence": 1,
            "traces": [],
            "moduleStats": {},
        },
...
def _live_agent_piano_summary(value, *, depth=0):
    if value is None or isinstance(value, (bool, int, float)):
        return value
    if isinstance(value, str):
        return value[:240]
    if isinstance(value, list):
        sample = [_live_agent_piano_summary(item, depth=depth + 1) for item in value[:5]]
    ...
    if isinstance(value, dict):
        ...
        for index, key in enumerate(sorted(value.keys(), key=lambda item: str(item))):
            if index >= 16:
                summary["truncated"] = True
                break
            summary[str(key)] = _live_agent_piano_summary(value.get(key), depth=depth + 1)
        return summary
    return str(value)[:240]


def _live_agent_piano_trace_start():
    return {"at": _utc_now_iso(), "perf": time.perf_counter()}


def _normalize_live_agent_piano_runtime(raw):
    source = raw if isinstance(raw, dict) else {}
    retention = _normalize_int(
        source.get("traceRetention"),
        LIVE_AGENT_LOOP_DEFAULTS["pianoRuntimeTraceRetention"],
        minimum=len(LIVE_AGENT_PIANO_MODULES),
        maximum=1000,
    )
    runtime = {
        "schemaVersion": LIVE_AGENT_PIANO_RUNTIME_TRACE_VERSION,
...
def _live_agent_piano_runtime_state(state):
    runtime = _normalize_live_agent_piano_runtime(state.get("pianoRuntime") if isinstance(state, dict) else None)
    state["pianoRuntime"] = runtime
    return runtime


def _live_agent_piano_append_trace(state, module_name, turn, started, *, inputs=None, outputs=None, decisions=None, gaps=None, status="completed", error=None):
    if module_name not in LIVE_AGENT_PIANO_MODULES or not isinstance(state, dict):
        return None
    runtime = _live_agent_piano_runtime_state(state)
    now_iso = _utc_now_iso()
    ...
    trace = {
        ...
        "inputs": _live_agent_piano_summary(inputs or {}),
        "outputs": _live_agent_piano_summary(outputs or {}),
        "decisions": _live_agent_piano_summary(decisions or {}),
        "gaps": clean_gaps,
    }
    if isinstance(error, dict):
        trace["error"] = _live_agent_piano_summary(error)
    elif error:
        trace["error"] = str(error)[:240]
    retention = _normalize_int(runtime.get("traceRetention"), LIVE_AGENT_LOOP_DEFAULTS["pianoRuntimeTraceRetention"], minimum=len(LIVE_AGENT_PIANO_MODULES), maximum=1000)
...
def _live_agent_piano_record_skipped_modules(state, turn, module_names, reason, details=None):
    rows = []
    for module_name in module_names:
        rows.append(_live_agent_piano_append_trace(
            state,
            module_name,
            turn,
            _live_agent_piano_trace_start(),
...
def _normalize_live_agent_loop_state(raw):
    ...
    if isinstance(raw, dict):
        ...
        if isinstance(raw.get("pianoRuntime"), dict):
            state["pianoRuntime"] = _normalize_live_agent_piano_runtime(raw["pianoRuntime"])
    ...
    state["pianoRuntime"] = _normalize_live_agent_piano_runtime(state.get("pianoRuntime"))
    return state
...
def live_agent_loop_tick(*, reason="timer", force=False, dry_run=False):
    with _live_agent_loop_lock:
        ...
        for agent in _live_agent_loop_order_agents_for_turn(enabled_agents, scheduler.get("lastAgentId")):
            ...
            orchestrator_started = _live_agent_piano_trace_start()
            ...
            try:
                ...
                memory_started = _live_agent_piano_trace_start()
                memory = agent_state.get("memory") if isinstance(agent_state.get("memory"), dict) else {}
                _live_agent_piano_append_trace(
                    state,
                    "memory",
                ...
                reflection_started = _live_agent_piano_trace_start()
                ...
                social_started = _live_agent_piano_trace_start()
                ...
                if active:
                    ...
                    _live_agent_piano_record_skipped_modules(state, turn, ["planning", "conversation", "actionExecution", "outcomeAwareness"], "active-behavior", skipped)
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "skipped", "skipReason": "active-behavior"}, decisions={"selectedAgentId": agent_id, "processed": True}, status="completed")
                    break
                ...
                if state.get("worldClientRequired") and not world_client.get("active") and not force:
                    ...
                    _live_agent_piano_record_skipped_modules(state, turn, ["planning", "conversation", "actionExecution", "outcomeAwareness"], "world-client-inactive", skipped)
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "skipped", "skipReason": "world-client-inactive"}, decisions={"selectedAgentId": agent_id, "processed": True}, status="completed")
                    break
                ...
                if next_allowed and now_epoch < next_allowed and not force:
                    ...
                    _live_agent_piano_record_skipped_modules(state, turn, ["planning", "conversation", "actionExecution", "outcomeAwareness"], "cooldown", skipped)
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "skipped", "skipReason": "cooldown"}, decisions={"selectedAgentId": agent_id, "processed": True}, status="completed")
                    break

                planning_started = _live_agent_piano_trace_start()
                ...
                if not selected:
                    ...
                    result["skipped"].append(skipped)
                    _live_agent_piano_append_trace(state, "planning", turn, planning_started, inputs={"availableAffordanceCount": len([item for item in (perception.get("affordances") or []) if item.get("available")])}, outputs={"selected": None, "decision": decision}, decisions=agent_state.get("lastDecision"), gaps=["no-available-loop-target"], status="completed")
                    ...
                    _live_agent_piano_record_skipped_modules(state, turn, ["conversation", "actionExecution", "outcomeAwareness"], "no-available-loop-target", skipped)
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "skipped", "skipReason": "no-available-loop-target"}, decisions={"selectedAgentId": agent_id, "processed": True}, status="completed")
                    break
                ...
                action_execution_started = _live_agent_piano_trace_start()
                if dry_run:
                    ...
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "completed", "loopActionId": action_def["id"], "planId": plan.get("id") if isinstance(plan, dict) else None}, decisions={"selectedAgentId": agent_id, "processed": True}, status="completed")
                    break
                ...
                if ok:
                    ...
                    _live_agent_piano_record_skipped_modules(state, turn, ["conversation", "outcomeAwareness"], "action-request-failed", error_row)
                    ...
                    _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "failed", "failureReason": "action-request-failed"}, decisions={"selectedAgentId": agent_id, "processed": True, "retry": retry}, gaps=["action-request-failed"], status="failed")
            except Exception as e:
                _live_agent_loop_stat(agent_state, "errors")
                _live_agent_loop_stat(state, "errors")
                ...
                _live_agent_piano_record_skipped_modules(state, turn, ["conversation", "actionExecution", "outcomeAwareness"], "turn-runtime-error", {"message": str(e), "retry": retry})
                _live_agent_piano_append_trace(state, "orchestrator", turn, orchestrator_started, inputs={"reason": reason, "force": force, "dryRun": dry_run}, outputs={"turnStatus": "failed", "failureReason": "turn-runtime-error"}, decisions={"selectedAgentId": agent_id, "processed": True, "retry": retry}, gaps=["turn-runtime-error"], status="failed", error=error_body)
            break
