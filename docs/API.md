...
For the planned backend-owned autonomous resident architecture, see [LIVE-AGENT-MODE-SPEC.md](LIVE-AGENT-MODE-SPEC.md). The current world-action API remains the compatibility foundation, but future Live Agent execution should make the server authoritative for turns, tool calls, movement progress, object-use side effects, and replayable animation events. The Live Agent Mode architecture uses the [reference repo](https://github.com/EmergenceAI/Emergence-World) as guidance for embodied persistent residents, tool-only world mutation, location-gated capabilities, memory/relationship context, visible event dispatch, and alive-world metrics. That guidance is scoped to Live Agent Mode and ClawMind, not normal manual world editing; unsafe or destructive reference tools remain proposal-only until My Virtual World has typed visible executors and rollback.
